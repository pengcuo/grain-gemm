...
def load_report(path):
    ...
    if len(set(expected_sizes)) != len(expected_sizes):
        raise ValueError("expected_sizes must not contain duplicates")
    ...
    if type(group_size) is not int or group_size not in (32, 64, 128, 256):
        raise ValueError("expected group size 32, 64, 128, or 256")
    timing = data["timing"]
    ...
    for case in reports:
        ...
        if case["sglang_config"] != {
            "block_m": 32, "block_n": 64, "num_warps": 4, "num_stages": 2,
        } or case["sglang_config_autotuned"] is not False:
            raise ValueError("expected the fixed, untuned SGLang baseline configuration")
        config = case.get("grain_kernel_config")
        if config is not None and not isinstance(config, dict):
            raise ValueError("grain_kernel_config must be a dictionary or null")
        ...
        ratios = {
            "grain_speedup_over_bf16": medians["torch_mm_bf16"] / medians["grain_int8_gemm"],
            "grain_speedup_over_sglang": medians["sglang_int8_gemm"] / medians["grain_int8_gemm"],
            "sglang_speedup_over_bf16": medians["torch_mm_bf16"] / medians["sglang_int8_gemm"],
        }
...
def grain_backend_label(reports):
    backends = {(case.get("grain_kernel_config") or {}).get("backend") for case in reports}
    native_names = {"cuda", "cute", "native", "cutlass", "cuda_cute", "cuda/cute"}
    if backends and backends.issubset(native_names):
        return "GrainGEMM CUDA / CuTe INT8 (TOPS)"
    if backends == {"triton"}:
        return "GrainGEMM Triton INT8 (TOPS)"
    if len(backends) > 1:
        return "GrainGEMM INT8, mixed backends (TOPS)"
    return "GrainGEMM INT8 (TOPS)"
...
def plot_report(data, output, svg=None):
    ...
    fig.suptitle(
        f"GrainGEMM G{data['group_size']}: custom INT8 GEMM performance",
...
def main():
    ...
    parser.add_argument("--input", type=Path, required=True, help="completed compare_grain.py JSON aggregate")
