...
Run one shape per process (default: three M256 shapes and 16 square sizes).
...
SHAPES = [(256, 1024, 1536), (256, 1536, 1024), (256, 2048, 1536)] + [
    (n, n, n) for n in range(1024, 16385, 1024)
]
...
def render(output_dir):
    ...
    lines += ['', 'Speedup = CuTe latency / CUTLASS latency; greater than 1 means CUTLASS is faster.', '',
              ...
              'both native backends use INT8 mma.sync and cp.async. CUTLASS candidates include the upstream '
              'FP32 exchange epilogue, BF16 shared exchange, and direct BF16 pair stores with register '
              'or asynchronous scale loading. It does not invoke the direct CuTe kernel.', '',
