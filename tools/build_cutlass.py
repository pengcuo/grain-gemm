...
def main():
    ...
    if not re.fullmatch(r"sm_[0-9]+[af]?", args.arch):
        parser.error("--arch must be a CUDA architecture such as sm_121")
    ...
    source_dir = ROOT / "src/grain_gemm/kernels/csrc"
    ...
    with tempfile.TemporaryDirectory(prefix="cutlass-build-", dir=output_dir) as temporary:
        ...
        command = [nvcc, "-O3", "-std=c++17", "--expt-relaxed-constexpr", f"-arch={args.arch}",
                   ...
                   "--ptxas-options=-v",
                   str(source_dir / "grain_cutlass.cu"), "-o", str(built)]
    ...
    metadata = dict(
        ...
        source_sha256={path.name: hashlib.sha256(path.read_bytes()).hexdigest()
                       for path in sorted(source_dir.glob("grain_cutlass*"))
