"""Shape-derived LLM prefill G256 benchmark on GB10 using the installed package.
...
def main():
    ...
    parser.add_argument('--output', type=Path, required=True, help='New result path outside benchmarks/results')
    ...
    if (ROOT / 'benchmarks/results').resolve() in args.output.resolve().parents:
        parser.error('Write new measurements outside the archived benchmarks/results directory')
    if args.output.exists() and not args.resume:
        parser.error('Output exists; use a new path or --resume for an incomplete matching run')
