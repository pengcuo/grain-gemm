...
GPU measurements require an NVIDIA GB10 device with compute capability 12.1
(SM121). The entry point checks both before loading native kernels or creating
results. Thor and other GPUs need separately validated experiments; CPU-only
result validation remains available without a GPU.
