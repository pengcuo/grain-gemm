...
EXPECTED_CONFIGS = ([dict(backend="cutlass", config_id=i) for i in range(8)]
...
def main():
    ...
    audit.require(scope["candidate_counts"] == dict(cute=17, cutlass=8, triton=9),
                  "scope candidate counts differ from 17/8/9")
    audit.require(scope["group_size"] == 256 and scope["output"] == "bfloat16",
                  "scope is not G256/BF16")
    script = root / "compare_cutlass.py"
    ...
    audit.require(candidates == EXPECTED_CONFIGS[25:], "nine Triton candidates differ from expected set")
    for source in (script, candidates_file):
        snapshot = root / "measurement_sources" / source.name
        audit.require(snapshot.is_file() and sha(snapshot) == sha(source),
                      f"script snapshot mismatch: {source.name}")
    ...
    for path in files:
        try:
            ...
            audit.require(attempted == expected_config_counter,
                          f"{label}: candidate set must cover all 34 configurations exactly once")
            for row in skipped:
            ...
            for backend in BACKENDS:
                eligible = [row for row in tune if row["config"]["backend"] == backend]
    ...
    print(json.dumps(dict(
        ...
        missing_shapes=missing, expected_candidate_attempts=18*34,
